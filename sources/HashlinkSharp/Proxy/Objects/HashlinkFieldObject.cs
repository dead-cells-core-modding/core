...
namespace Hashlink.Proxy.Objects
{
        ...
        where T : unmanaged
    {
        ...
        {
            void* ptr = hl_obj_lookup((HL_vdynamic*)HashlinkPointer, hashedName, out var ftype);
            if(ptr == null)
            {
                ptr = hl_obj_lookup_extra((HL_vdynamic*)HashlinkPointer, hashedName);
                if(ptr != null)
                {
                    return HashlinkMarshal.ConvertHashlinkObject(ptr);
                }
                throw new MissingFieldException(new string(hl_type_str(Type)), new string(hl_field_name(hashedName)));
            }
